...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    if (b.fotos && b.fotos.length) {
      for (const f of b.fotos) {
        await pool.query('INSERT INTO ocorrencia_fotos (ocorrencia_id, dados) VALUES (?,?)', [req.params.id, f]);
      }
    }
    if (b.video) {
      await pool.query('INSERT INTO ocorrencia_videos (ocorrencia_id, dados) VALUES (?,?)', [req.params.id, b.video]);
    }
    res.json({ ok: true });
